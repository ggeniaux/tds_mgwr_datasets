---
title: "datasets for article : Top-Down Scale Approaches for Multiscale GWR with Locally Adaptive Bandwidths"
author: "Ghislain Geniaux"
date: "2025-30-09"
output: html_document
---

First, Intall R, Rstudio and Python if necessary

Then, using a terminal console, install this environement and libraries : 
...
Theses series of python and R chunk allow to download data from PYSAL. 
...
```{python berlin}
explain('berlin')
prenz = pd.read_csv('/Users/geniaux/Documents/Boulot/programme/mgwrsar2/MGWR_DATA_EXAMPLE/prenzlauer.csv')
...
data = pd.read_csv("/Users/geniaux/Documents/Boulot/programme/mgwrsar2/MGWR_DATA_EXAMPLE/kc_house_data.csv")
...
```{python airbnb}
airbnb = pd.read_csv("/Users/geniaux/Documents/Boulot/programme/mgwrsar2/MGWR_DATA_EXAMPLE/AB_NYC_2019.csv")
...
save(datasets,a_coords,a_X,a_Y,b_coords,b_X,b_Y,c_coords,c_X,c_Y,g_coords,g_X,g_Y,k_coords,k_X,k_Y,t_coords,t_X,t_Y,file='/Users/geniaux/Documents/Boulot/programme/mgwrsar2/MGWR_DATA_EXAMPLE/alldatsets.Rdata')

```

```{r perfomance predictive}
library(caret)
library(xgboost)
library(GWmodel)
library(reticulate)
...
library(mgwrsar)
load(file='/Users/geniaux/Documents/Boulot/programme/mgwrsar2/MGWR_DATA_EXAMPLE/alldatsets.Rdata')
...
models_name=c('xgboost','xgboost_xy','GWR','tds_mgwr')
datasets_name<-c("DVF","king_house","airbnb")#,"king_house",'airbnb')
datasets_name<-c("georgia" ,"clearwater", "tokio","berlin","DVF")# ,"king_house", "airbnb")

#datasets_name='tokio'
models_name=c('xgboost','xgboost_xy','GWR','tds_mgwr','atds_mgwr','multiscale_gwr')
models_name=c('GWR','python_mgwr')
...
RMSE=results_in$DVF
AICc=AICc_in$DVF
#cat(" rmse model=", rmse(preds[,'Y']-preds[,'GWR_model']),"\n")
cat(" rmse model=", rmse(preds[,'Y']-preds[,'python_mgwr_model']),"\n")
apply(RMSE,2,mean)
apply(AICc,2,mean)

save(results,results_in,AICc_in,file='/Users/geniaux/Documents/Boulot/programme/mgwrsar2/MGWR_DATA_EXAMPLE/results_python_DVF.Rdata')

save(results,results_in,AICc_in,file='/Users/geniaux/Documents/Boulot/programme/mgwrsar2/MGWR_DATA_EXAMPLE/results_all.Rdata')

#### RELANCER  pour multiscale_gwr georgia, clearwater , berlin


length(nchar(readLines("/Users/geniaux/Documents/Boulot/programme/mgwrsar2/simu_tds/result_final/TAB_9/GWRDVF.txt", encoding = "UTF-8")))-1

####  RELANCER DVF AVEC AICC TRUE pour tous les model sauf GWR
...
Y=results[[d]][,1]
RES=results[[d]][,-1]
apply(RES[,],2,function(x) rmse(Y-x))


for(d in datasets_name) {
Y=results[[d]][,1]
RES=results[[d]][,-1]
cat(d,' ',round(apply(RES[,c('GWR_estim','multiscale_gwr_model','tds_mgwr_model','atds_mgwr_model')],2,function(x) rmse(Y-x)),digits=3))#
}

for(d in datasets_name) cat(d,' ',round(apply(results_in[[d]][,models_name],2,mean),digits=3))

for(d in datasets_name) cat(d,' ',round(apply(AICc_in[[d]][,models_name],2,mean),digits=3))


save(results,results_in,AICc_in,file='/Users/geniaux/Documents/Boulot/programme/mgwrsar2/MGWR_DATA_EXAMPLE/results_4petits.Rdata')

save(results,results_in,AICc_in,file='/Users/geniaux/Documents/Boulot/programme/mgwrsar2/MGWR_DATA_EXAMPLE/results_all.Rdata')

save(results,results_in,AICc_in,file='/Users/geniaux/Documents/Boulot/programme/mgwrsar2/MGWR_DATA_EXAMPLE/results_3gros.Rdata')
load(file='/Users/geniaux/Documents/Boulot/programme/mgwrsar2/MGWR_DATA_EXAMPLE/results1.Rdata')
```
